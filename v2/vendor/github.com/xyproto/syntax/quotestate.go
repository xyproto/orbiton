...
func (q *QuoteState) StartedMultiLineComment() bool {
	return q.startedMultiLineComment
}
...
func (q *QuoteState) StoppedMultiLineComment() bool {
	return q.stoppedMultiLineComment
}
...
func (q *QuoteState) ContainsMultiLineComments() bool {
	return q.containsMultiLineComments
}
...
func CheckMultiLineString(trimmedLine string, inCodeBlock bool) (bool, bool) {
	...
	if trimmedLine == "\"\"\"" || trimmedLine == "'''" { // only 3 letters
		...
		if strings.Count(trimmedLine, "\"\"\"")+strings.Count(trimmedLine, "'''") > 0 &&
			(strings.Count(trimmedLine, "\"\"\"")%2 != 0 || strings.Count(trimmedLine, "'''")%2 != 0) {
			inCodeBlock = !inCodeBlock
		}
		if inCodeBlock {
			foundDocstringMarker = true

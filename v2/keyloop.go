	...
	for !e.quit {
		...
		RETURN_PRESSED_AI_DONE:
			...
			if trimmedLine == "private:" || trimmedLine == "protected:" || trimmedLine == "public:" {
				...
				break
			} else if e.mode == mode.Arduino || e.mode == mode.C || e.mode == mode.Cpp || e.mode == mode.Shader || e.mode == mode.Zig || e.mode == mode.Java || e.mode == mode.JavaScript || e.mode == mode.Kotlin || e.mode == mode.TypeScript || e.mode == mode.D || e.mode == mode.Hare || e.mode == mode.Jakt || e.mode == mode.Scala {

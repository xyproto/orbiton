...
type Editor struct {
	...
	bookSavedBottomBarVisible   bool        // saved BottomStatusBar.Visible from before book mode
	...
	statusMode                  bool        // display a status bar at all times at the bottom of the screen (legacy, being replaced)
	TopStatusBar                *StatusBar  // top status bar (used in book text mode)
	BottomStatusBar             *StatusBar  // bottom status bar (used in all modes)
...
func (e *Editor) Copy(withLines bool) *Editor {
	...
	e2.debugShowConsole = e.debugShowConsole
	e2.debugConsoleOutput = e.debugConsoleOutput
	e2.binaryFile = e.binaryFile
	e2.wrapWhenTyping = e.wrapWhenTyping
	e2.addSpace = e.addSpace
	e2.debugStepInto = e.debugStepInto
	e2.debugLastStepWasInstruction = e.debugLastStepWasInstruction
	...
	e2.bookSavedBottomBarVisible = e.bookSavedBottomBarVisible
...
func (e *Editor) RestoreFrom(snap Editor, lines map[int][]rune, pos Position) {
	...
	bookSavedBottomBarVisible := e.bookSavedBottomBarVisible
	...
	*e = *(snap.Copy(withLines))
	...
	e.bookSavedBottomBarVisible = bookSavedBottomBarVisible
...
func (e *Editor) Delete(c *vt.Canvas, useBlockMode bool) {
	...
	deleteThisRune := func() bool {
		...
		if _, ok := e.lines[y]; !ok || lineLen == 0 || (lineLen == 1 && unicode.IsSpace(e.lines[y][0])) {
			if useBlockMode {
				return true // skip empty/whitespace lines in block mode
			}
	...
	if e.DataY() < LineIndex(e.Len()) {
		...
		if e.DownEnd(c) != nil {
			...
			if !e.AfterEndOfDocument() {
				...
				redraw := e.ScrollDown(c, status, 1, canvasHeight)
				e.redraw.Store(redraw)

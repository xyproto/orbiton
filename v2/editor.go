...
func (e *Editor) CommentOn(commentMarker string) {
	...
	case mode.Config, mode.HCL, mode.Ini, mode.FSTAB, mode.Nix, mode.TOML, mode.YAML:
		break // space == ""
	default:
		space = " "
	}
...
func (e *Editor) JoinLineWithNext(c *vt.Canvas) bool {
	...
	if marker := e.SingleLineCommentMarker(); e.ProgrammingLanguage() && strings.HasPrefix(e.TrimmedLine(), marker) {

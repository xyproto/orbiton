	...
	for y = LineIndex(0); y < LineIndex(numLinesToDraw); y++ {
		...
		if e.syntaxHighlight && !envNoColor {
			...
			if textWithTags, err = syntax.AsText([]byte(escapeFunction(line)), e.mode); err != nil {
				...
				case mode.Config, mode.CMake, mode.JSON, mode.Ini, mode.FSTAB:
					if !strings.HasPrefix(trimmedLine, singleLineCommentMarker) && (strings.Contains(trimmedLine, "/*") || strings.HasSuffix(trimmedLine, "*/")) {
						// No highlight
						coloredString = line
					} else if (e.mode == mode.Ini || e.mode == mode.Config || e.mode == mode.FSTAB) && strings.HasPrefix(trimmedLine, ";") {
						// Commented out
				...
				case mode.Ada, mode.Agda, mode.Elm, mode.Garnet, mode.Haskell, mode.Lua, mode.Nmap, mode.SQL, mode.Teal, mode.Terra: // not for OCaml and Standard ML
					trimmedLine = strings.TrimSpace(line)
					if strings.HasPrefix(trimmedLine, "--") {
						// Handle single line comments
						coloredString = unEscapeFunction(e.MultiLineComment.Start(line))

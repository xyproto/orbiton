...
func NewEditor(tty *TTY, c *Canvas, fnord FilenameOrData, lineNumber LineNumber, colNumber ColNumber, theme Theme, origSyntaxHighlight, discoverBGColor, monitorAndReadOnly, nanoMode, createDirectoriesIfMissing, displayQuickHelp, noDisplayQuickHelp bool) (*Editor, string, bool, error) {
	if inVTEGUI {
	...
	if !e.readOnly && (!specificLetter || editTheme) {
		...
		default:
			if (env.Has("XTERM_VERSION") && !inVTEGUI && env.Str("ALACRITTY_LOG") == "") || env.Str("TERMINAL_EMULATOR") == "JetBrains-JediTerm" {
				...
				if r, g, b, err := GetBackgroundColor(tty); err == nil && r+g+b > 2 { // success and the background is not dark
					b := true
					initialLightBackground = &b
					if editTheme {
						e.setBlueEditTheme(*initialLightBackground)
...
func (e *Editor) PrepareEmptySaveAndRemove(c *Canvas, tty *TTY) (bool, error) {

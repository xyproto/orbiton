...
func adjustSyntaxHighlightingKeywords(m mode.Mode) {
	...
	case mode.Go:
		...
		delKeywords = []string{"None", "assert", "build", "char", "def", "def", "die", "done", "fi", "final", "finally", "from", "get", "in", "include", "is", "last", "let", "match", "mut", "next", "no", "pass", "redo", "rescue", "ret", "retry", "set", "template", "then", "this", "when", "where", "while", "yes"}
	case mode.HIDL:
		clearKeywords()

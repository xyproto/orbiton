...
var (
	asmWords = []string{"_start", "add", "bits", "db", "dd", "div", "dq", "dw", "equ", "global", "inc", "int", "lea", "mov", "org", "rol", "ror", "section", "shl", "shr", "sub", "syscall", "xor"}
...
func adjustSyntaxHighlightingKeywords(m mode.Mode) {
	...
	case mode.Go:
		...
		delKeywords = []string{"None", "build", "char", "def", "def", "die", "done", "fi", "from", "get", "in", "include", "is", "let", "mut", "next", "no", "pass", "redo", "rescue", "ret", "retry", "set", "template", "then", "this", "when", "where", "while", "yes"}
	case mode.HIDL:
		clearKeywords()
	...
	case mode.Shell:
		addKeywords = []string{"--force", "-f", "cmake", "configure", "do", "endif", "fdisk", "for", "gdisk", "in", "make", "mv", "ninja", "rm", "rmdir", "setopt", "while"}
		delKeywords = []string{"#else", "#endif", "default", "double", "exec", "float", "fn", "get", "install", "long", "new", "no", "pass", "property", "ret", "super", "template", "var", "with"}

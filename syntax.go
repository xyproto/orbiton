...
func adjustSyntaxHighlightingKeywords(mode Mode) {
	...
	case modeGo:
		...
		delKeywords = []string{"None", "build", "char", "get", "include", "mut", "pass", "set", "template"}
	case modeLisp:
	...
	case modeRust:
		addKeywords = []string{"assert_eq", "fn", "impl", "loop", "mod", "out", "panic", "usize"}
		delKeywords = []string{"build", "done", "end", "next"}
	case modeLua:
		syntax.Keywords = make(map[string]struct{})
	...
	default:
		delKeywords = append(delKeywords, []string{"require", "build", "package", "super", "type", "operator"}...)
	}

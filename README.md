...
* Press `right, down, left` or `left, down, right` in quick succession followed by `up` to just quit, `down` to save and quit or type in a command. This is handy to avoid [the emacs pinky](http://xahlee.info/emacs/emacs/emacs_pinky.html).
